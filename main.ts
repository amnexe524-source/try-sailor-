...
serve(async (req) => {
  ...
  if (method === "POST" && path === "/") {
    ...
    if (alertsRaw) {
      ...
      await KV.set(["alerts_fired"], JSON.stringify(fired));
    }
  ...
  if (method === "POST" && path === "/stats") {
    ...
    if (newWave && newWave !== prevWave && newWave !== "Not in Dungeon") {
      await KV.set(["last_wave"], newWave);
      const chatId = await kvGet("chat_id");
  ...
  if (path === "/config") {
    ...
    if (method === "POST") {
      let body: unknown;
      try { body = await req.json(); } catch { return jsonRes({ error: "invalid json" }, 400); }
      await KV.set(["ui_config"], JSON.stringify(body));
      return jsonRes({ ok: true });
    }
  }
  ...
  if (path === "/discord-config") {
    ...
    if (method === "POST") {
      let body: unknown;
      try { body = await req.json(); } catch { return jsonRes({ error: "invalid json" }, 400); }
      await KV.set(["discord_config"], JSON.stringify(body));
      return jsonRes({ ok: true });
    }
  }
  ...
  if (path === "/icons") {
    ...
    if (method === "POST") {
      ...
      await KV.set(["custom_icons"], JSON.stringify(existing));
      return jsonRes({ ok: true });
    }
    if (method === "DELETE") {
      let body: Record<string, string>;
  ...
  if (path === "/alerts") {
    ...
    if (method === "POST") {
      let body: unknown;
      try { body = await req.json(); } catch { return jsonRes({ error: "invalid json" }, 400); }
      await KV.set(["alerts"], JSON.stringify(body));
      await KV.set(["alerts_fired"], "{}");
      return jsonRes({ ok: true });
    }
  }
  ...
  if (method === "GET" && path === "/app") {
    return htmlRes(getDashboardHTML(url.hostname));
  }
...
// DASHBOARD HTML
// ════════════════════════════════════════════════════════
function getDashboardHTML(hostname: string): string {
